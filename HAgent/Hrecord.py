...
from mitmproxy import flow as flowss
...
class HAgent:
    ...
    def __init__(self):
        ...
        self.loader = Loader(ctx)
        self.filter = flowfilter.TFilter
        self.loader.add_option(name="flow_filter",
                               typespec=str,
                               default="Stop mock",
                               help="Add a filter")
        # 创建socket对象
    ...
    def load(loader: Loader):
        loader.add_option("flow_filter", str, "", "init filter.")
        ...
        while True:
            ...
            if data_from_server == "STOP":
                ...
                try:
                    ...
                    # 模拟弱网指令
                    elif "method" in data_json and data_json["method"] == "MockNetwork":
                        self.config["Network"] = data_json
                        ctx.options.flow_filter = self.config["Network"]["filter"]
                        self.filter = flowfilter.parse(ctx.options.flow_filter)
                    # 停止模拟指令
                    elif "method" in data_json and data_json["method"] == "UnMockNetwork":
                        ctx.options.flow_filter = "Stop mock"
                        if "Network" in self.config:
                            self.config.pop("Network")
    ...
    def request(self, flow: mitmproxy.http.HTTPFlow):
        # 模拟弱网，判断是否上传过滤条件
        filter_text = ctx.options.flow_filter
        if filter_text and filter_text != "Stop mock":
            network_operator = self.config["Network"]["operator"]
            if flowfilter.match(self.filter, flow):
                if network_operator[0] == 2:  # 网络延迟
                    delay_in = self.config["Network"]["lag"]["in"]
                    delay_time = self.config["Network"]["lag"]["value"]
                    if delay_in == 2:   # 请求延时
                        time.sleep(delay_time)
                if network_operator[1] == 2:  # 网络丢包
                    # kill掉flow，当前请求将不会转发到其目的地
                    drop_in = self.config["Network"]["drop"]["in"]
                    drop_rate = self.config["Network"]["drop"]["value"]
                    if drop_in == 2:    # 请求丢包
                        ran = random.random()
                        if ran <= drop_rate:    # 概率丢包
                            flow.kill()
        ...
        if len(filtration) > 0:
            if filtration_type in str(flow.request.method) or filtration_type == "ALL":
                ...
                if filter_text and filter_text != "Stop mock":
                    ...
                    if flowfilter.match(self.filter, flow):
                        if network_operator[0] == 2:  # 网络延迟
                            delay_out = self.config["Network"]["lag"]["out"]
                            delay_time = self.config["Network"]["lag"]["value"]
                            if delay_out == 2:  # 请求延时
                                time.sleep(delay_time)
                        if network_operator[1] == 2:  # 网络丢包
                            # kill掉flow，当前请求将不会转发到其目的地
                            drop_out = self.config["Network"]["drop"]["out"]
                            drop_rate = self.config["Network"]["drop"]["value"]
                            if drop_out == 2:  # 请求丢包
                                ran = random.random()
                                if ran <= drop_rate:  # 概率丢包
                ...
                if self.is_connect():
                    try:
                        ...
                        self.console.log(f"Error client send message: {e}")
